...
/* harmony export */ });
const Hammer = __webpack_require__(840);
const featherlight = __webpack_require__(204);
const featherlightGallery = __webpack_require__(522);
const defaults = {
...
class Lightbox {
  constructor(wrap) {
    ...
    this.gallery = this.wrap.querySelector(this.options.gallery);
    this.hammer = new Hammer(wrap[0]);
    this.hammer.addEventListener("click tap press", this.onTapEvt);
    this.hammer.addEventListener("pan swipe", this.onSwipeEvt);
  }
  onTapEvt = function (evt) {
    this.gallery.featherlightGallery(this.options.gallerySettings);
  };
  onSwipeEvt = function (evt) {
    this.gallery.unbind("click.featherlight");
  };
}
...
class Nav {
  constructor(wrap) {
    ...
    this.dopdownTrigger = this.wrap.querySelector(this.options.dropdownTrigger);
    this.dopdown = this.$wrap.querySelector(this.options.dropdown);
    this.minWidth = 1024;
    this.navTrigger.addEventListener("click", this.onNavTriggerClick);
    this.dopdownTrigger.addEventListener("click", this.onDropdownTriggerClick);
  }
  onNavTriggerClick = function () {
  ...
  onDropdownTriggerClick = function (evt) {
    if (!evt.target.classList.contains("js-nav-dropdown-link")) {
      evt.preventDefault();
  ...
  openDropdown = function () {
    ...
    this.isOpen = true;
    this.dopdownTrigger.classList.add(this.options.dropdownOpenClass);
  };
  closeDropdown = function () {
    if (!this.isOpen) return;
    this.isOpen = false;
    this.dopdownTrigger.classList.remove(this.options.dropdownOpenClass);
  };
}
module.exports = Nav;
...
const sectionReady = function ({
  $node,
  returnInstances = false
}) {
  const instances = returnInstances || [];
  sectionReady.components.forEach(function (component) {
    node[0].getElementsByClassName(component.class).forEach(function (element) {
      const instance = new component.Component(element);
...
sectionReady.addComponents = function (components) {
  sectionReady.components = sectionReady.components.concat(components);
};
sectionReady.destroy = function (instances) {
...
/* harmony export */ });
const Flickity = __webpack_require__(442);
const defaults = {
...
class Slider {
  constructor(wrap) {
    ...
    this.thumbnails = document.querySelectorAll(this.options.thumbnails);
    this.toggleOpenLabel = this.sliderToggle.querySelector(this.options.toggleOpenLabel);
    this.toggleCloseLabel = this.sliderToggle.querySelector(this.options.toggleCloseLabel);
    this.setupSlider();
    this.sliderToggle.addEventListener("click", this.toggleThumbnails);
    this.thumbnails.addEventListener("click", ".thumbnail", this.onThumbnailClick);
  }
  setupSlider = function () {
    const settings = this.wrap.getAttribute(this.options.flickityDataSettings);
    settings = JSON.parse(settings) || {};
    settings = {
      ...this.options.flickitySettings,
      ...settings
    };
    this.flickity = new Flickity(this.wrap[0], settings);
  };
  getIndex = function (selector) {
    this.thumbnails.querySelectorAll(".thumbnail").forEach(function (thumb, index) {
      if (selector === thumb) return index;
    });
  };
  onThumbnailClick = function (evt) {
    evt.preventDefault();
    let index = this.getIndex(evt.currentTarget);
...
( function( window, factory ) {
  ...
  if (  true && module.exports ) {
    // CommonJS
    module.exports = factory( __webpack_require__(131) );
  } else {
    // browser global
    window.Flickity = window.Flickity || {};
...
( function( window, factory ) {
  ...
  if (  true && module.exports ) {
    // CommonJS
    module.exports = factory( __webpack_require__(680) );
  } else {
    // browser global
...
// const FocusFix = require("./focus-fix")
const sectionReady = __webpack_require__(922);
const app = {};
...
app.setup = function () {
  ...
  // A list of class names matched to components, e.g.:
  {
    ...
    class: "js-lightbox",
    Component: __webpack_require__(683)
  }]);
